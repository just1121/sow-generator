...
def load_vectorstore():
    vectorstore_dir = os.getenv('VECTORSTORE_PATH', '/Users/kc/myproject_env/vectorstore.faiss')
    vectorstore_path = Path(vectorstore_dir)
    print(f"Checking for vectorstore at: {vectorstore_path}")
    print(f"Directory exists: {vectorstore_path.is_dir()}")
    print(f"Contents of directory: {list(vectorstore_path.glob('*'))}")
...
def initialize_vectorstore():
    ...
    try:
        ...
        doc_path = '/Users/kc/myproject_env/documents/'
        ...
        vectorstore.save_local("preprocessed_vectorstore")
...
async def generate_sow():
    with st.spinner("Generating SOW..."):
        try:
            ...
            # Generate labor costs text (ADD THIS LINE HERE)
            ...
            # Executive Summary
            [Using the Strategic Context, craft a compelling narrative that explains the project's purpose, goals, and desired outcomes. Focus on the business value and strategic importance.]

            # 1. Description of Services
            [Detail the technical approach, methodologies, and specific services using the Technical Approach section]
            ...
            IMPORTANT: 
            1. Do not include any title or heading at the beginning of your response. Start directly with the Executive Summary. Only mention the client name where it's specifically relevant in the content of each section.
            2. Maintain RWS's professional yet approachable tone throughout. Focus on clear communication of technical concepts while emphasizing partnership with the client.
            """

            vectorstore_results = await asyncio.to_thread(query_vectorstore, prompt)
            
            prompt += "\n\nRelevant information from our knowledge base:\n"
            for result in vectorstore_results:
                prompt += f"- {result.page_content[:200]}...\n"
...
def get_audio_input(question, key):
    ...
    duration_mapping = {
        ...
        "timeline": 4,
        "primary_goal": 10,
        "primary_challenge": 10,
        "existing_systems": 10,
        "intangible_deliverables": 10,
        "tangible_deliverables": 10,
        "kpis": 15,
        "risks_mitigation": 15,
        "communication_protocols": 15,
        "additional_statements": 20
    }

    duration = duration_mapping.get(key, 8)

    if st.session_state.input_method == 'Audio':
        if st.button("Speak", key=f"speak_{key}"):
            audio_data = record_audio(duration=duration)
            audio_content = io.BytesIO(audio_data).read()
            transcription = transcribe_audio(audio_content)
            st.session_state[key] = transcription

    user_input = st.text_area("Transcription Output", value=st.session_state.get(key, ""), key=f"display_{key}", height=100, label_visibility="collapsed")
...
def main():
    ...
    # Project Details with Conditional Questions
    st.subheader("Project Details")
    
    # Handle main radio button questions
    for question in st.session_state.questions['project_details']['main_questions']:
        question["answer"] = st.radio(
            question["question"],
            ["Yes", "No"],
            key=f"pd_main_{question['id']}",
            index=0 if st.session_state.get(f"pd_main_{question['id']}") == "Yes" else 1
        )
        
        # Show phase details if multiple phases selected
        if question['id'] == 1 and question["answer"] == "Yes":
            st.markdown("##### Phase Details")
            for phase_q in st.session_state.questions['project_details']['phase_details']:
                phase_q["answer"] = get_audio_input(phase_q["question"], f"phase_{phase_q['id']}")
                
        # Show technical details if technical requirements selected
        if question['id'] == 2 and question["answer"] == "Yes":
            st.markdown("##### Technical Details")
            for tech_q in st.session_state.questions['project_details']['technical_details']:
                tech_q["answer"] = get_audio_input(tech_q["question"], f"tech_{tech_q['id']}")
    ...
    # Check Vectorstore Button
    if st.button("Check Vectorstore Content", key="check_vectorstore"):
