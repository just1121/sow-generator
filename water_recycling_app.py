        ...
        if 'labor_costs' not in st.session_state:
            st.session_state.labor_costs = {
                'roles': [
                    ...
                    {'role': 'Professional Travel Time', 'rate': 110.00, 'hours': 0},
                ...
                # 'total': 0 # This seems related to a global sum, handled differently now
            }
...
def load_environment():
    ...
    if not google_api_key:
        st.error("Google API key not found. Please add GOOGLE_API_KEY as an environment variable.")
        st.stop()
    
    # Check for speech credentials only if speech is enabled
    if speech_available and not google_creds:
        st.error("Google Application Credentials not found for speech services. Please add GOOGLE_APPLICATION_CREDENTIALS as an environment variable.")
        st.stop()
        ...
        if content:
            ...
            for paragraph in paragraphs[1:]:
                ...
                if "5. Basis for Compensation" in paragraph:
                    ...
                    # This section is intentionally left blank if "5. Basis for Compensation" is found,
                    # as the content for "4. Basis for Compensation" (generated by generate_section_5_costs)
                    # is the one that should be formatted into the DOCX.
                    # The actual formatting for costs happens in the
                    # `elif "4. Basis for Compensation" in paragraph:` block.
                ...
                if "**Executive Summary**" in paragraph:
                    ...
                    # Skip content if we're in section 2 and see markdown from AI that we will replace
                    if any(marker in paragraph for marker in [
                        "Contractor will provide Deliverables under this SOW as described here:",
                        "**2. Deliverables**", # This is the header we are adding, so don't skip if it's this line
                        "**Deliverable", # Could be part of AI generated content
                        "| Milestone |", # AI table
                        "|-----------|", # AI table
                        "| Description |", # AI table
                        "| Target Date |"  # AI table
                    ]) and not paragraph.strip() == "**2. Deliverables**": # Don't skip our own header
                        continue
                    ...
                    for i, (del_key, deliverable) in enumerate(st.session_state.deliverables.items(), 1):
                        if deliverable.get('description', '').strip():
                            ...
                            # Add deliverable title on first line
                            ...
                            # Add completion date on separate line
                            if target_date:
                                date_str = target_date.strftime('%B %d, %Y')
                                date_text = f"Target completion date: **{date_str}**"
                            else:
                                date_text = f"Target completion date: **(No date provided)**"
                            
                            p_date = doc.add_paragraph()
                            add_markdown_runs(p_date, date_text, apply_base_body_style)
                ...
                # Add comprehensive skip conditions - but don't skip our new deliverable format
                    ...
                    narrative = (f"The estimated cost for completion of this Statement of Work is ${total_cost:,.0f}. "
                        ...
                        if isinstance(deliverable.get('labor_costs'), dict):
                            ...
                            for role, details in deliverable['labor_costs'].items():
                                if isinstance(details, dict) and details.get('hours', 0) > 0:
                                    ...
                                    row_cells[2].text = f"${details['rate']:.0f}/hr"
                                    row_cells[3].text = f"{details['hours']:.2f}"
                                    row_cells[4].text = f"${details['total']:,.0f}"
                            ...
                            p_labor_total.add_run(f"${total_deliverable_labor_cost:,.0f}").bold = True
                            ...
                            if has_additional_costs:
                                        ...
                                        for item in equipment_rentals['items']:
                                            if 'weeks' in item and 'rate_per_week' in item:
                                                ...
                                                if amount > 0 or desc:
                                                    ...
                                                    row_cells[1].text = f"{desc} - {weeks} {week_text} @ ${rate_per_week:,.0f}/week"
                                ...
                                if additional_costs.get('truck_days', {}).get('enabled', False):
                                    ...
                                    row_cells[1].text = f"{days:.1f} days @ ${rate:.0f}/day"
                                    row_cells[2].text = f"${amount:,.0f}"
                                    deliverable_additional_total += amount
                    ...
                    if st.session_state.rental_rates.get('has_rentals', False) and rental_total > 0:
                        ...
                        for item in st.session_state.rental_rates['items']:
                            if item['description'] and item['qty'] > 0 and item['rate'] > 0:
                                ...
                                row_cells[3].text = f"${item['rate']:.0f}"
                                row_cells[4].text = f"${item_total:,.0f}"
                        
                        doc.add_paragraph()  # Add space before total
                        p = doc.add_paragraph()
                        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                        p.add_run("Total Rental Costs: ")
                        p.add_run(f"${rental_total:,.0f}").bold = True
                    ...
                    row_cells[2].text = f"${materials_total:,.0f}"
                    ...
                    if deliverable_totals:
                        ...
                        if materials_total > 0:
                            ...
                            row_cells[1].text = f'${materials_total:,.0f}'
        ...
        if deliverable_key in st.session_state.deliverables:
            ...
            for role, details in deliverable.get('labor_costs', {}).items():
                if details['hours'] > 0:
                    ...
                    labor_lines.append(f"  {role}: {details['hours']:.1f} hours at ${details['rate']:.0f}/hr (${cost:,.0f})")
                    total_cost += cost
...
def create_entries_record():
    ...
    try:
                ...
                if deliverable.get('description'):
                    ...
                    if deliverable.get('additional_costs'):
                            ...
                            if items:
                                for idx, item in enumerate(items, 1):
                                    ...
                                    if 'weeks' in item and 'rate_per_week' in item:
                                        ...
                                        doc.add_paragraph(f"  {idx}. {desc} — Weeks: {weeks}, Rate/Week: ${rpw:,.0f}, Total: ${total:,.0f}")
                                    else:
                                        amt = item.get('amount', 0)
                                        doc.add_paragraph(f"  {idx}. {desc} — Amount: ${amt:,.0f}")
                            else:
                                # Old format
                                amt = er.get('amount', 0)
                                doc.add_paragraph(f"  Amount: ${amt:,.0f}")
                        ...
                        if ac.get('mileage', {}).get('enabled', False):
                            ...
                            doc.add_paragraph(f"Mileage: {miles} miles at ${rate:,.0f}/mile — Total: ${total:,.0f}")
                        ...
                        if ac.get('truck_days', {}).get('enabled', False):
                            ...
                            doc.add_paragraph(f"Truck Days: {days} days at ${rate:,.0f}/day — Total: ${total:,.0f}")
        ...
        doc.add_heading(f'Additional Costs: ${total_additional_costs:,.0f}').bold = True
        
        # Display the costs breakdown
        if total_deliverable_additional_costs > 0:
            doc.add_paragraph(f"Deliverable-Specific Additional Costs: ${total_deliverable_additional_costs:,.0f}")
        if materials_total > 0:
            doc.add_paragraph(f"Global Materials Total: ${materials_total:,.0f}")
        ...
        doc.add_paragraph(f"Materials Cost (+ 25% markup): ${expenses_safe.get('materials_cost', 0):,.0f}")
        doc.add_paragraph(f"Materials Total: ${materials_total:,.0f}")
        doc.add_paragraph(f"Material Costs (project-wide): ${materials_total:,.0f}")
        ...
        if labor_costs_safe:
            ...
            for role, rate in labor_costs_safe.items():
                ...
                try:
                    ...
                    row_cells[1].text = f"${rate_value:.0f}/hr"
                except Exception:
...
def generate_section_5_costs():
    ...
    # Add narrative
    client_name = st.session_state.questions['client']["answer"].strip()
    section += (f"The estimated cost for completion of this Statement of Work is ${total_project_cost:,.0f}. "
                f"The tables below detail the estimated efforts required. Material changes to the "
                f"SOW will be agreed upon in writing and may constitute a change in basis for "
                f"compensation increasing or decreasing accordingly.\n\n"
                f"Efforts not explicitly listed in the table below are the responsibility of {client_name}. "
            ...
            for role, details in deliverable['labor_costs'].items():
                if isinstance(details, dict) and details.get('hours', 0) > 0:
                    section += (f"| {role} | {details.get('description', '')} | "
                              f"${details['rate']:.0f}/hr | {details['hours']:.2f} | "
                              f"${details['total']:,.0f} |\n")
            ...
            if 'additional_costs' in deliverable:
                ...
                if (additional_costs.get('equipment_rentals', {}).get('enabled', False) or
                            ...
                            for item in equipment_rentals['items']:
                                if 'weeks' in item and 'rate_per_week' in item:
                                    ...
                                    if amount > 0 or desc:
                                        ...
                                        section += f"| Equipment Rentals | {desc} - {weeks} {week_text} @ ${rate_per_week:,.0f}/week | ${amount:,.0f} |\n"
                                        deliverable_additional_total += amount
                                else:
                                    # Old amount structure
                                    amount = item.get('amount', 0)
                                    desc = item.get('description', '')
                                    if amount > 0 or desc:
                                        section += f"| Equipment Rentals | {desc} | ${amount:,.0f} |\n"
                                        deliverable_additional_total += amount
                        else:
                            # Old format compatibility
                            amount = equipment_rentals.get('amount', 0)
                    ...
                    if additional_costs.get('mileage', {}).get('enabled', False):
                        ...
                        rate = additional_costs['mileage']['rate']
                        amount = miles * rate
                        section += f"| Mileage | {miles:.1f} miles @ ${rate:.3f}/mile | ${amount:,.0f} |\n"
                        deliverable_additional_total += amount
                    ...
                    if additional_costs.get('truck_days', {}).get('enabled', False):
                        ...
                        rate = additional_costs['truck_days']['rate']
                        amount = days * rate
                        section += f"| Truck Days | {days:.1f} days @ ${rate:.0f}/day | ${amount:,.0f} |\n"
                        deliverable_additional_total += amount
                    ...
                    if additional_costs.get('travel', {}).get('enabled', False):
                        ...
                        if 'items' in travel_data:
                            ...
                            for item in travel_data['items']:
                                ...
                                desc = item.get('description', '')
                                if amount > 0 or desc:
                                    section += f"| Travel | {desc} | ${amount:,.0f} |\n"
                                    deliverable_additional_total += amount
                        else:
                            # Old format compatibility
                            amount = travel_data.get('amount', 0)
...
def main():
    ...
    for i in range(num_deliverables):
        ...
        with st.expander(f"Deliverable {i + 1} Related Costs"):
            ...
            if equipment_rental_enabled:
                ...
                for idx, item in enumerate(rental_items):
                    ...
                    with col1:
                        item_desc = st.text_input(
                            ...
                            placeholder="e.g., RO system rental"
                        )
                    ...
                    with col2:
                        weeks = st.selectbox(
                            f"Weeks",
                            options=list(range(1, 53)),  # 1-52 weeks
                            index=item.get('weeks', 1) - 1,
                            key=f"rental_weeks_{deliverable_key}_{idx}"
                        )
                        item['weeks'] = weeks
                    
                    with col3:
                        rate_per_week = st.number_input(
                            f"Rate/week ($)",
                            value=float(item.get('rate_per_week', 0.0)),
                            min_value=0.0,
                            step=50.0,
                            key=f"rental_rate_{deliverable_key}_{idx}"
                        )
                        item['rate_per_week'] = rate_per_week
                        item_total = weeks * rate_per_week
                        equipment_total += item_total
                    
                    with col4:
                        # Show calculated total
                        st.text(f"Total: ${item_total:,.0f}")
                ...
                if equipment_total > 0:
                    st.markdown(f"*Equipment Rentals Total: ${equipment_total:,.0f}*")
            ...
            if mileage_enabled:
                ...
                with col3:
                    ...
                    st.text(f"Total: ${mileage_total:.0f}")
            ...
            if truck_days_enabled:
                ...
                with col3:
                    ...
                    st.text(f"Total: ${truck_total:.0f}")
            ...
            if travel_enabled:
                ...
                # Show travel total
                if travel_total > 0:
                    st.markdown(f"*Travel Total: ${travel_total:,.0f}*")
        ...
        with st.expander(f"Labor Categories - Deliverable {i + 1}"):
            labor_roles_template = st.session_state.labor_costs['roles']
            
            # Initialize total cost for this deliverable
            total_deliverable_cost = 0.0
            
            # Ensure labor_costs for the deliverable is initialized
            ...
            for role_template in labor_roles_template:
                role_name = role_template['role']
                role_rate = role_template['rate']
                deliverable_labor_entry = st.session_state.deliverables[deliverable_key]['labor_costs'].get(role_name, {})
                ...
                with col1:
                    st.text(role_name)
                with col2:
                    st.text(f"${role_rate:.0f}/hr")
                with col3:
                    hours_key = f"hours_{deliverable_key}_{role_name.replace(' ', '_').replace('/', '_')}"
                    
                    # Get existing hours from session state
                    existing_hours = deliverable_labor_entry.get('hours', 0)
                ...
                with col4:
                    current_total = role_rate * hours
                    st.text(f"${current_total:,.0f}")
                    total_deliverable_cost += current_total
                
                # Show description field if hours > 0
                if hours > 0:
                    desc_key = f"work_desc_{deliverable_key}_{role_name.replace(' ', '_').replace('/', '_')}"
                    existing_desc = deliverable_labor_entry.get('description', '')
                    work_description = st.text_area(
                        "Description of Work",
                        value=existing_desc,
                        key=desc_key,
                        placeholder=f"Enter description for {role_name} work...",
                        label_visibility="collapsed"
                    )
                else:
                    work_description = ""
                ...
                if deliverable_key not in st.session_state.deliverables:
                    st.session_state.deliverables[deliverable_key] = {} # Should have been initialized earlier
                st.session_state.deliverables[deliverable_key]['labor_costs'][role_name] = {
                    'hours': hours,
                    'rate': role_rate,
                    'total': current_total,
    ...
    for i in range(num_deliverables):
        ...
        if deliverable_key in st.session_state.deliverables:
            ...
            st.session_state['total_labor_cost'] += deliverable_labor_cost
            
            # Calculate additional costs for this deliverable
            deliverable_additional_cost = 0.0
            if 'additional_costs' in deliverable:
                additional_costs = deliverable['additional_costs']
    ...
    with col2:
        ...
        st.text(f"Materials Total: ${materials_total:.0f}")
        
        # Total material costs
        total_project_wide_additional_costs = materials_total
        st.text(f"**Material Costs (project-wide): ${total_project_wide_additional_costs:.0f}**")
    ...
    with col2:
        st.markdown("**Amount**")
        st.markdown(f"${st.session_state['total_labor_cost']:,.0f}")
        st.markdown(f"${total_deliverable_additional_costs:,.0f}")
        st.markdown(f"${total_project_wide_additional_costs:,.0f}")
        st.markdown("---")
